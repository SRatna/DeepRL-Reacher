...
In this project, I have worked with a distributed version of the Unity environment which contains 20 identical agents, each with its own copy of the environment. 

To solve this version it is required to get an average score of +30 (over 100 consecutive episodes, and over all agents).  Specifically,
...
To set up a python environment to run the code in this repository, follow the instructions below.
...
1. Download the environment from one of the links below. Select the environment that matches the operating system being used:
...
Open `Continuous_Control.ipynb` notebook and run the cells to train the agent. Alternatively, you can also test the agent performance.